...
function HC_sim_test_flat(; Nx=256, Ny=256, Nz=64)
    H = 1.0
    Lx = 4*H
    Ly = Lx
    ...
    # Create flat bottom grid (NO immersed boundary)
    ...
    # NO pressure_solver for flat bottom - use default FFT solver
    ...
    println("\n" * "="^60)
    println("SUCCESS! Flat bottom working on GPU!")
    println("="^60)
