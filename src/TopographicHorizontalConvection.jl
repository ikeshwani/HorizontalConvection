...
function TopographicHorizontalConvection(; Ra=2e11, h₀_frac=0.6, output_writer=true, advection=true, architecture=GPU())
    ...
    χ = @at (Center, Center, Center) κ * (∂x(b)^2 + ∂y(b)^2 + ∂z(b)^2)
    
    ke = @at (Center, Center, Center) 1/2 * (u^2 + v^2 + w^2)
    pe = PotentialEnergy(model)

    ζ = @at (Center, Center, Center) ∂z(u) - ∂x(w)
    ...
    if output_writer
        ...
        simulation.output_writers[:buoyancy] = NetCDFOutputWriter(model, (; b, χ),
        ...
        simulation.output_writers[:section_snapshots] = NetCDFOutputWriter(model, (; b, χ, ζ, ke, pe),
        ...
        simulation.output_writers[:zonal_time_means] = NetCDFOutputWriter(model, (; b=b_avg_x),
                                                              schedule = AveragedTimeInterval(2, window=1),
                                                              filename = filename,
                                                              with_halos = true,
                            							      global_attributes = global_attributes,
                                                              overwrite_existing = true)
