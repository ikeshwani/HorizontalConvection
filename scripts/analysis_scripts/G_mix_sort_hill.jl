...
outfile  = joinpath(data_dir, "Gmix_regions_RA1e8_seg1to19.nc")
mkpath(plot_dir)
...
# wet mask from time step 2 (avoids initialization zeros); hills are b==0
wet = Array(ds1["b"][:, :, :, 2]) .!= 0
close(ds1)
...
# ---- load segments 1–19, skipping overlapping time steps ----
println("loading b, χ, u from segments 1–19...")
...
function plot_gmix_regions(b_out, time, Gmix_regions, region_precomp, plot_dir)
    ...
    figname = "Gmix_density_regions_RA1e8_seg1to19.png"
