# region_contours_equilibrium.jl -- same analysis as region_contours.jl, but
# reads the actively-rebinned, equilibrium-only CODF file produced by
# gmix_CODF_equilibrium.jl (Gmix_int_$nm/psi_int_$nm/etc. on a 201-bin axis
# built by pooling the final segment's own snapshots) instead of the full-run
# file's coarse, single-snapshot axis. This is the direct test of whether that
# fix actually resolves last week's case-study bug (a real point at b=-0.355
# landing in an oversized terminal bin, -0.652 nonsensical contour, ~30% error
# vs Gmix_from_FBN averaged over only 3 bins).
#
# Every output filename gets an "_equilibrium" suffix so this run cannot
# collide with / overwrite the original region_contours.jl figures already in
# this directory (single_contour_efficiency_hill*.png, zoom_in_gmix_hill*.png).
#
# Everything else (contour-threshold logic, F_BN backsolve, region boundaries)
# is untouched from region_contours.jl -- only the gmix_file source and output
# filenames differ.
...
# the ONLY substantive change vs region_contours.jl: point at the
# equilibrium-only, actively-rebinned CODF file instead of the full-run one.
gmix_file     = joinpath(data_dir, "Gmix_quantile_regions_CODF_$(tag)_$(Ra_str)_seg$(last_seg)_equilibrium_nbins801.nc")
buoyancy_file = joinpath(data_dir, "buoyancy_seg$(last_seg).nc")
isfile(gmix_file) || error("equilibrium gmix file not found: $gmix_file (run gmix_CODF_equilibrium.jl first)")

# # refuse to run if any output figure already exists -- never silently clobber
# outputs_to_check = vcat(["single_contour_efficiency_hill$(n)_equilibrium.png" for n in 1:3],
#                          ["zoom_in_gmix_hill$(n)_equilibrium.png" for n in 1:3])
# for fn in outputs_to_check
#     fp = joinpath(plot_dir, fn)
#     isfile(fp) && error("refusing to run: output figure already exists: $fp")
# end
...
function plot_hill_shell(plot_dir, hill_name, blo, bhi, left_nm, left_xlo, right_nm, right_xhi,
    ...
    outpng = joinpath(plot_dir, "single_contour_efficiency_$(hill_name)_equilibrium_nbins801.png")
    save(outpng, fig)
...
# ---- zoom_in_gmix_$(hill)_equilibrium.png: the actual Gmix_int(b) curve
# shape around [b_in,b_out], with b_in/b_out/b_mid marked -- generalizes the
# original single_contour_efficiency.jl diagnostic so we can SEE why
# Gmix_from_FBN (the exact average over the whole interval) does or doesn't
# match Gmix_meas at any single point, rather than just inferring it from
# %diffs. Scatter shows every real point; the line is broken across gaps
# wider than gap_thresh so a sparse stretch of axis can't manufacture a fake
# cliff.
function plot_gmix_zoom(plot_dir, hill_name, b_centers, Gmix_curve, blo, bhi, bmid)
    ...
    fig = Figure()
    ax = Axis(fig[1,1], title="$hill_name gmix versus buoyancy (equilibrium, actively rebinned)", xlabel="gmix", ylabel="b")
    ...
    outpng = joinpath(plot_dir, "zoom_in_gmix_$(hill_name)_equilibrium.png")
    save(outpng, fig)
    println("saved -> $outpng")
    return outpng
end
